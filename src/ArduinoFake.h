...
#define ArduinoFakeReset() getArduinoFakeContext()->reset()

#define ArduinoFakeInstance(mock, ...) \
  getArduinoFakeContext()->mock(__VA_ARGS__)

#define ArduinoFakeMock(mock) new mock##FakeProxy(ArduinoFakeInstance(mock))

#define _ArduinoFakeGetMock(mock) getArduinoFakeContext()->Mocks->mock
...
#define _ArduinoFakeGetSPIBus() _ArduinoFakeGetMock(SPIBus)
...
class ArduinoFakeContext {
 public:
  ...
  //  _ArduinoFakeInstanceGetter2(SPIBus, SPIBus);

  ArduinoFakeContext() { this->reset(); }
